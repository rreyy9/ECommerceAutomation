...
{
    /// <summary>
    /// UI Tests executed on Chrome browser
    /// </summary>
    [TestClass]
    public class UITests_Chrome : BaseTest
    {
        [TestInitialize]
        public override void Setup()
        {
            Log("Setting up Chrome browser for test execution");
            Driver = WebDriverFactory.CreateDriver(WebDriverFactory.BrowserType.Chrome);
            Log("Chrome browser initialized successfully");
        }

        /// <summary>
        /// Test Case 1: Login User with Correct Email and Password - Chrome
        /// </summary>
        [TestMethod]
        [TestCategory("UI")]
        [TestCategory("Login")]
        [TestCategory("Chrome")]
        public void Test01_LoginWithValidCredentials_Chrome()
        {
            Log("Starting Test: Login User with Correct Email and Password (Chrome)");
            ...
            try
            {
                ...
                Log("Test PASSED: User successfully logged in on Chrome");
            }
            catch (Exception ex)
            {
                Log($"Test FAILED on Chrome: {ex.Message}");
                throw;
            }
        }

        /// <summary>
        /// Test Case 2: Search Product - Chrome
        /// </summary>
        [TestMethod]
        [TestCategory("UI")]
        [TestCategory("Products")]
        [TestCategory("Chrome")]
        public void Test02_SearchProduct_Chrome()
        {
            Log("Starting Test: Search Product (Chrome)");
            ...
            try
            {
                ...
                Log("Test PASSED: Product search completed successfully on Chrome");
            }
            catch (Exception ex)
            {
                Log($"Test FAILED on Chrome: {ex.Message}");
